...
- **📅 今日複習(SRS)**:收藏咗嘅句子/生字按間隔重複排程(1→3→7→14→30→60 日),每日話你知要溫邊幾張,撳「記得/唔記得」自動調整。
- **🎭 情境對話**:見工面試、睇醫生、餐廳點餐、電話投訴、旅行問路 — AI 做返嗰個角色同你演練。
- **📖 生字簿**:撳 AI 回覆入面唔識嘅字 → 彈出繁中解釋 + 例句 → 一掣加入生字簿,一齊入埋 SRS。
- **🎙 跟讀評分**:複習時跟住讀,Whisper 轉錄後逐字比對,話你知邊個字讀唔清(綠=啱,紅=漏/錯)。
- **💬 多對話**:可以開多個對話存檔,今日傾嘅嘢聽日繼續;串流回覆逐字顯示。
- **📊 用量儀表板**:底部顯示今日/本月大約用咗幾多 token 同語音次數。
- **🔉 語音本機保存**:生成過嘅語音存入 IndexedDB,同一句永遠只生成一次,離線都播到。

## 雲端同步(可選)

喺 Vercel 加 Upstash Redis integration(Marketplace 一鍵),會自動注入 `KV_REST_API_URL` / `KV_REST_API_TOKEN`,收藏就會自動跨裝置同步(「★ 收藏」頁有「☁ 同步」掣)。唔設定嘅話 app 照用,純本機。

## 收藏 / 系統性複習

- 對話中每句(更正、完整正確版本、AI 回應)旁邊有 **☆ 收藏掣**,撳一下記低(連時間)。
- 頂部 **★ 收藏** 入面翻查所有收藏,見到每句幾時記低,逐句可 🔊。
- 可以打剔揀句子 → **▶ 播放已選** 連續聽,或者 **⬇ 下載 MP3** 將揀咗嘅句子合併成一個 MP3 檔,循環重溫。
- 收藏存喺 browser `localStorage`(同一部機/browser)。匯出時已播過嘅句子會重用快取音訊,慳 points。
